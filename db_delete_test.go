...
func Test_pgProfileDb_delete_with_phone_numbers_succeeds(t *testing.T) {
	checkIntegrationTest(t)
	db := newTestPgProfileDb(t)
	defer newTestPgProfileDb(t)
	_, err := db.Exec(context.Background(), `
...
func Test_pgProfileDb_delete_inexistant_user_returns_error(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "no profile found for keycloak id \"11000000-0000-0000-0000-000000000000\"", err.Error())
}
