...
// Fetch multiple profiles based in parameters provided
func (db *pgProfileDB) getMultipleProfiles(ctx context.Context, intSkip int, intLimit int) ([]user, error) {
	...
	rows, err := db.Query(ctx, `
		...
		wants_ten_group,
		name_of_ten_group
	FROM users LIMIT $1 OFFSET $2`, intLimit, intSkip)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
